...
function renderChart() {
  ...
  const maxCount = max(props.stats, d => d.count) ?? 1
  const y = scaleLinear()
  ...
  g.append('g')
    .call(axisLeft(y).ticks(3).tickSize(0).tickFormat(format('d')))
    .call(g => g.select('.domain').remove())
    .selectAll('text')
    .style('fill', 'var(--color-muted-foreground)')
    .style('font-size', '9px')
...
<template>
  <div ref="containerRef" class="border-b bg-muted/30 px-4 py-2">
    ...
    <svg
      ref="svgRef"
      role="img"
      aria-label="Bar chart showing message frequency over the last 24 hours"
      class="w-full"
