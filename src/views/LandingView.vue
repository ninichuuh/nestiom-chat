  ...
  <div class="relative min-h-screen overflow-hidden bg-background">
    ...
    <section class="relative z-10 mx-auto max-w-5xl px-6 pb-24 pt-20 md:pt-32">
      <div class="max-w-2xl">
        <div class="reveal">
          <div
            class="mb-6 inline-flex items-center gap-2 rounded-full border bg-background/80 px-3.5 py-1.5 text-xs font-medium text-muted-foreground backdrop-blur-sm">
      ...
      <div class="reveal reveal-delay-1 mt-16 md:absolute md:right-6 md:top-28 md:mt-0 md:w-85">
        <div class="rounded-2xl border bg-background/90 p-4 shadow-2xl shadow-foreground/5 backdrop-blur-md">
          <!-- Mock header -->
          <div class="flex items-center gap-3 border-b pb-3">
            <div class="flex h-8 w-8 items-center justify-center rounded-full bg-primary/10 text-xs font-semibold">AK
            </div>
            <div>
          ...
          <div class="space-y-2.5 py-4">
            <div class="flex justify-start">
              <div class="max-w-[75%] rounded-2xl rounded-bl-md bg-muted px-3.5 py-2 text-xs">Hey! How's the project
                going?</div>
            </div>
            <div class="flex justify-end">
              <div class="max-w-[75%] rounded-2xl rounded-br-md bg-foreground px-3.5 py-2 text-xs text-background">Just
                finished the real-time sync</div>
            </div>
            <div class="flex justify-start">
              <div class="max-w-[75%] rounded-2xl rounded-bl-md bg-muted px-3.5 py-2 text-xs">That's amazing! Can I test
                it?</div>
            </div>
            <div class="flex justify-end">
              <div class="max-w-[75%] rounded-2xl rounded-br-md bg-foreground px-3.5 py-2 text-xs text-background">Open
                it in two tabs and try it!</div>
            </div>
            <!-- Typing indicator -->
            <div class="flex justify-start">
          ...
          <div class="flex items-center gap-2 border-t pt-3">
            <div class="flex-1 rounded-md border bg-muted/50 px-3 py-1.5 text-xs text-muted-foreground">Type a
              message...</div>
    ...
    <section class="relative z-10 mx-auto max-w-5xl px-6 py-24">
      ...
      <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <div v-for="(feature, i) in features" :key="feature.title"
          class="reveal group rounded-xl border bg-background/70 p-5 backdrop-blur-sm hover:border-foreground/20 hover:shadow-lg hover:shadow-foreground/2"
          :style="{ transitionDelay: `${i * 80}ms` }">
          <div
            class="mb-3 flex h-9 w-9 items-center justify-center rounded-lg border bg-muted/50 transition-colors group-hover:bg-foreground group-hover:text-background">
        ...
        <div class="mx-auto max-w-xl text-center">
          ...
          <div class="mt-8 flex items-center justify-center gap-4">
            <Button size="lg" variant="outline"
              class="border-background/20 bg-transparent text-background hover:bg-background/10" as-child>
              <RouterLink :to="{ name: 'login' }">Sign In</RouterLink>
            </Button>
